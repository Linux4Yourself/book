...
`--with-glibc-version=2.35` - эта опция указывает версию glibc, которая будет использоваться в целевой системе. Она не относится к glibc хост-системы, потому что всё, скомпилированное с помощью gcc (первый проход), будет выполняться в среде chroot, которая изолирована от glibc хост-системы.
